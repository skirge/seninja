...
class SymbolicVisitor(BNILVisitor):
    ...
    def visit_LLIL_INTRINSIC(self, expr, level):
        log.log_debug(f"LLIL_INTRINSIC:{expr.intrinsic.name} at {hex(expr.address)}")
    ...
    def visit_LLIL_CALL(self, expr,level):
        ...
        if sym_from_type_refs is None and sym_from_code_refs is None and symbolic(dest):
            log.log_debug(f"symbolic dest {dest} at {expr}")
        ...
        if dest_fun_name in library_functions:
            log.log_debug(f"Running {dest_fun_name} builtin handler")
            res = library_functions[dest_fun_name](
                self.executor.state, self.executor.view)
            log.log_debug(f"{dest_fun_name} builtin handler returned {res}")
            try:
    ...
    def visit_LLIL_TAILCALL(self, expr,level):
        ...
        if dest_fun_name in library_functions:
            ...
            dest = self.executor.arch.get_return_address(self.executor.state)
            if symbolic(dest):
                log.log_debug(f"symbolic return address {dest} at {expr}")
                raise UnconstrainedIp(self.executor.ip)
    ...
    def visit_LLIL_JUMP_TO(self, expr,level):
        ...
        if num_ips == 256:
            ...
            log.log_debug(f"Probably unconstrained IP  at JUMP_TO:{expr} - num_ips:{num_ips}")
            raise UnconstrainedIp(self.executor.ip)
    ...
    def visit_LLIL_RET(self, expr,level):
        ...
        if symbolic(dest):
            ...
            if num_ips == 256:
                self.executor.put_in_errored(
                    self.executor.state, "Probably unconstrained IP")
                log.log_debug(f"Probably unconstrained IP at RET:{expr}, num_ips:{num_ips}")
                raise UnconstrainedIp(self.executor.ip)
            if num_ips == 0:
                self.executor.put_in_errored(
                    self.executor.state, "No valid destination")
