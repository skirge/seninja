...
def log_error_handler(state: State, view):  # only concrete
    string_p = get_arg_k(state, 3, state.arch.bits() // 8, view)
...
def streamBuffSeek_handler(state: State, view):
    global buff_fd
    if buff_fd is None:
        print("Opening buffer")
        streamBuffCreate_handler(state, view)
    offset = get_arg_k(state, 2, state.arch.bits(),  view)
    whence = get_arg_k(state, 3, 4, view)
    print(f"streamBuffSeek_handler:offset={offset}, whence={whence}")

    if symbolic(offset):
        actual_offset = state.solver.max(offset)
    else:
        actual_offset = offset.value
    #state.os.seek(buff_fd, actual_offset, whence)
    state.os.seek(buff_fd, actual_offset)
    return BVV(actual_offset, 32)

def streamBuffPos_handler(state: State, view):
    global buff_fd
    if buff_fd is None:
        print("Opening buffer")
        streamBuffCreate_handler(state, view)
    print(f"streamBuffPos_handler:")
    res = state.os.ftell(buff_fd)
    return BVV(res, 32)

def streamBuffLen_handler(state: State, view):
    global buff_fd
    if buff_fd is None:
        print("Opening buffer")
        streamBuffCreate_handler(state, view)
    print(f"streamBuffLen:")
    res = state.os.file_size(buff_fd)
    return BVV(res, 32)

def streamBuffGet_handler(state: State, view):
    global buff_fd
    if buff_fd is None:
        print("Opening buffer")
        streamBuffCreate_handler(state, view)
    s_p = get_arg_k(state, 2, state.arch.bits() // 8, view)
    size = get_arg_k(state, 3, 4, view)
    print(f"streamBuffGet_handler:s_p={s_p}, size={size}")
    if symbolic(size):
        actual_size = state.solver.max(size)
        max_size = state.executor.bncache.get_setting(
            "models.max_size_symb_string")
    ...
    for i in range(actual_size):
        ...
        size_read += l
    state.mem.store(s_p + actual_size, BVV(0, 8))
    return BVV(size_read, 32)

def streamBuffCreate_handler(state: State, view):
    global buff_fd
    print("streamBuffCreate_handler")
    buff_fd = state.os.open("buffer", "r--")
    return BVV(buff_fd, 32)

def streamBuffRelease_handler(state: State, view):
    global buff_fd
    print("streamBuffRelease_handler")
    state.os.close(buff_fd)
    return BVV(0, 32)
